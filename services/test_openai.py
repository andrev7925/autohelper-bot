...
TRANSLATED_SCREENSHOT_TEXT = {
    "uk": "На жаль, не вдалося прочитати оголошення, тому що сайт блокує збір даних. Повторіть спробу, відправивши посилання ще раз, і якщо результат буде той самий — зробіть до 4 скриншотів оголошення (вид авто спереду, водійська частина салону, опис, пробіг, ціна, історія тощо) та надішліть їх через кнопку «Аналіз по фото».\n\n📌 Правила: доступно максимум 3 аналізи авто на день і максимум 4 скриншоти для одного аналізу.",
    "ru": "К сожалению, не удалось прочитать объявление, потому что сайт блокирует сбор данных. Повторите попытку, отправив ссылку ещё раз, и если результат будет тем же — сделайте до 4 скриншотов объявления (вид авто спереди, водительская часть салона, описание, пробег, цена, история и т. д.) и отправьте их через кнопку «Анализ по фото».\n\n📌 Правила: доступно максимум 3 анализа авто в день и максимум 4 скриншота для одного анализа.",
    "en": "Unfortunately, we couldn’t read the listing because the website is blocking data collection. Please try sending the link again, and if the result is the same — take up to 4 screenshots of the listing (front view, driver area, description, mileage, price, history, etc.) and send them using the “Photo Analysis” button.\n\n📌 Rules: maximum 3 car analyses per day and up to 4 screenshots for one analysis.",
    "es": "Lamentablemente, no se pudo leer el anuncio porque el sitio web bloquea la recopilación de datos. Intenta enviar el enlace nuevamente y, si el resultado es el mismo, toma hasta 4 capturas de pantalla del anuncio (vista frontal, zona del conductor, descripción, kilometraje, precio, historial, etc.) y envíalas con el botón «Análisis por foto».\n\n📌 Reglas: máximo 3 análisis de autos por día y hasta 4 capturas para un análisis.",
    "pt": "Infelizmente, não foi possível ler o anúncio porque o site bloqueia a coleta de dados. Tente enviar o link novamente e, se o resultado for o mesmo, tire até 4 capturas de tela do anúncio (vista frontal, área do condutor, descrição, quilometragem, preço, histórico etc.) e envie pelo botão «Análise por foto».\n\n📌 Regras: máximo de 3 análises de carros por dia e até 4 capturas para uma análise.",
    "tr": "Ne yazık ki ilan okunamadı çünkü site veri toplamayı engelliyor. Lütfen bağlantıyı tekrar gönderin; sonuç aynı olursa ilanın en fazla 4 ekran görüntüsünü alın (ön görünüm, sürücü bölümü, açıklama, kilometre, fiyat, geçmiş vb.) ve «Fotoğrafla Analiz» düğmesiyle gönderin.\n\n📌 Kurallar: günde en fazla 3 araç analizi ve bir analiz için en fazla 4 ekran görüntüsü.",
}
...
async def handle_media_or_forward(message: types.Message):
    lang = (getattr(message.from_user, "language_code", None) or "uk")[:2]
    media_hint = {
        "uk": "Для аналізу фото або скріншотів скористайтесь кнопкою нижче.\n\n📌 Правила: максимум 3 аналізи авто на день і максимум 4 скриншоти для одного аналізу.",
        "ru": "Для анализа фото или скриншотов используйте кнопку ниже.\n\n📌 Правила: максимум 3 анализа авто в день и максимум 4 скриншота для одного анализа.",
